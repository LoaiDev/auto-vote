...
function checkCompletion_2captcha(code, key, repeat) {
	...
	http.onreadystatechange = function() {
		if(http.readyState == 4 && http.status == 200){
			...
			if(response == messageNotReady_2captcha){
				setTimeout(function() {
...
function checkCompletion_anticaptcha(code, key, repeat, cid) {
	...
	http.onreadystatechange = function() {
		...
		if(http.readyState == 4 && http.status == 200){
			...
			let response = JSON.parse(http.responseText).solution.gRecaptchaResponse;
			if(JSON.parse(http.responseText).status=='ready'){
				clearInterval(cid);
				setCaptchaCode(response);
			}
		}
	}
	http.open("POST", url, true);
	http.setRequestHeader("Content-Type", "application/json");
	http.send(JSON.stringify(obj2));
}
...
function checkCompletion_bestcaptcha(code, key, repeat, cid) {
	//setupTextarea();
	...
	http.onreadystatechange = function() {
		if(http.readyState == 4 && http.status == 200){
			...
			if(response.includes("pending")){
				...
				setCaptchaCode(JSON.parse(http.responseText).gresponse);
			}
		}
	}
	http.open("GET", url, true);
...
function checkCompletion_endcaptcha(code, key, repeat, cid) {
...
function startWatching_bestcaptcha(code, key) {
	let initial = 7500, repeat = 5000;
	var cid = setInterval(function() {
		checkCompletion_bestcaptcha(code, key, repeat, cid);
	}, initial);
}
...
function startWatching_endcaptcha(code, key) {
	let initial = 7500, repeat = 5000;
	var cid = setInterval(function() {
		checkCompletion_endcaptcha(code, key, repeat, cid);
	}, initial);
}
	...
	http.onreadystatechange = function() {
		if(http.readyState == 4 && http.status == 200){
			...
			if(response.includes('OK|')==false){
				sendMessage("<b>"+response+"</b>")
			}else{
				startWatching_2captcha(response.substring(3), key);
			}
	...
	http.onreadystatechange = function() {
		if(http.readyState == 4 && http.status == 200){
			...
			if(response.includes('OK|')==false){
				...
				startWatching_rucaptcha(response.substring(3), key);
			}
		}else if(http.readyState == 4){
			sendMessage("Error from 2captcha.com");
		}
...
function makeRequest_anticaptcha(result){
	...
	http.onreadystatechange = function() {
		if(http.readyState == 4 && http.status == 200){
			let response = JSON.parse(http.responseText).taskId;
			if(JSON.parse(http.responseText).taskId==undefined){
				sendMessage("<b>"+JSON.parse(http.responseText).errorCode+"</b>")
			}else{
				startWatching_anticaptcha(response, key);
			}
		}else if(http.readyState == 4){
			sendMessage("Error from Anti-Captcha.com");
		}
	}
	http.open("POST", url, true);
	http.setRequestHeader("Content-Type", "application/json");
...
function makeRequest_bestcaptcha(result){
	...
	http.onreadystatechange = function() {
		if(http.readyState == 4 && http.status == 200){
			let response = JSON.parse(http.responseText).id;
			if(JSON.parse(http.responseText).id==undefined){
				sendMessage("<b>"+JSON.parse(http.responseText)+"</b>")
			}else{
				startWatching_bestcaptcha(response, key);
			}
		}else if(http.readyState == 4){
			sendMessage("Error from BestCaptchaSolver.com");
		}
	}
	http.open("POST", url, true);
	http.setRequestHeader("Content-Type", "application/json");
	http.send(JSON.stringify(obj_best));
}
...
function makeRequest_endcaptcha(result){
    ...
    action: 'xhttp',
    url: url,
    data: `username=${key.split(':')[0]}&password=${key.split(':')[1]}&type=4&token_params={"proxy":"","proxytype":"","googlekey":"${id}","pageurl":"${pageurl}"}`
	}, function(responseText) {
